...
/*	count */
	...
	$varresult_count = mysql_query($varquery_count) or die("Operation Failure: " . mysql_error());
	//
	//
	// skew: mysql_num_rows() returns the number of rows in a result set. This command  is only valid for SELECT statements. To retrieve the number of rows affected by the last INSERT, UPDATE or DELETE query, ie $varquery1,  use mysql_affected_rows(). note that if the link identifier isn't specified, the last link opened by mysql_connect()- $varconnect, is assumed (+ the largest prime number under 50).
	//	skew:
	...
	//	$varcount = mysql_num_rows($varresult_count) + $varskew;
	$varcount = mysql_num_rows($varresult_count);
/*	done */
?>