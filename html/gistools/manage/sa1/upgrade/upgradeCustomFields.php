...
foreach ($custom_fields_def as $mod => $fields) {
	...
	for ($fieldnum = 0; $fieldnum < count($fields); $fieldnum++) {
		...
		$meta_data = array_map("mysql_escape_string", $meta_data);
	...
	while ($custom_record = $db->fetchByAssoc($custom_records_res)) {
		...
		$custom_data = array_map("mysql_escape_string", $custom_data);
...
for ($i = 0; $i < count($queries); $i++) {
	...
	if(!$rowcount){
		$db->query($queries[$i]['query']) or log_error($queries[$i]['label'], "MySQL error: {$queries[$i]['label']} -- {$queries[$i]['mod']} -- {$queries[$i]['fieldnum']} -- {$queries[$i]['query']} -- " . mysql_error());
