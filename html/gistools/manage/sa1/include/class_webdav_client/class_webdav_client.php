...
/*
 CVS:
  $Id: class_webdav_client.php,v 1.1 2005/03/20 10:53:38 robert Exp $
  $Author: robert $
  $Date: 2005/03/20 10:53:38 $
  $Revision: 1.1 $
*/
/**
...
class webdav_client {
 ...
 /**
	* Set webdav server path.
	* @param string server
	*/
	function set_server_path($server_path) {
 ...
 /** 
	...
	*/
	function mkcol($path) {
		$this->_path = $this->_server_path.$this->_translate_uri($path);
		$this->_header_unset();
 ...
 /** 
	...
	*/
	function copy_file($src_path, $dst_path, $overwrite) {
	 ...
	 $this->_header_add(sprintf('Destination: http://%s%s%s', $this->_server,$this->_server_path, $this->_translate_uri($dst_path)));
	 if ($overwrite) {
...
/** 
	...
	*/
	...
	function move($src_path,$dst_path, $overwrite) {
		...
		$this->_path = $this->_server_path.$this->_translate_uri($src_path);
		$this->_header_unset();
			...
			switch($response['status']['status-code']) {
				case 200:
					...
					if (strcmp($response['header']['Content-Type'], 'text/xml; charset="utf-8"') == 0) {
						...
						print 'Missing Content-Type: text/xml header in response.<br> content-type='. $response['header']['Content-Type'].'<BR><br>';
					}
					return false;
