 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: Popup.php,v 1.5 2005/02/10 22:44:05 kashyap Exp $
...
$log = LoggerManager::getLogger('bug');
...
{
	$search_fields = Array("name", "number");

	$where_clauses = Array();

	append_where_clause($where_clauses, "name", "bugs.name");
	append_where_clause($where_clauses, "number", "bugs.number");
...
if (!isset($_REQUEST['html'])) {
	$form =new XTemplate ('modules/Bugs/Popup_picker.html');
	$log->debug("using file modules/Bugs/Popup_picker.html");
}
else {
	$log->debug("_REQUEST['html'] is ".$_REQUEST['html']);
	$form =new XTemplate ('modules/Bugs/'.$_REQUEST['html'].'.html');
	$log->debug("using file modules/Bugs/".$_REQUEST['html'].'.html');
}
...
if($_REQUEST['form'] == "TasksEditView")
{
	...
	$the_javascript .= "function set_return(bug_id, bug_name) {\n";
	$the_javascript .= "	window.opener.document.EditView.parent_name.value = bug_name;\n";
	$the_javascript .= "	window.opener.document.EditView.parent_id.value = bug_id;\n";
	$the_javascript .= "}\n";
...
elseif($_REQUEST['form'] == 'EditView')
{
	$the_javascript  = "<script type='text/javascript' language='JavaScript'>\n";
	$the_javascript .= "function set_return(bug_id, bug_name) {\n";
	$the_javascript .= "	window.opener.document.EditView.bug_name.value = bug_name;\n";
	$the_javascript .= "	window.opener.document.EditView.bug_id.value = bug_id;\n";
	$the_javascript .= "}\n";
	$the_javascript .= "</script>\n";
...
{
	$the_javascript  = "<script type='text/javascript' language='JavaScript'>\n";
	$the_javascript .= "function set_return(bug_id, account_name) {\n";
	$the_javascript .= "	window.opener.document.DetailView.bug_id.value = bug_id; \n";
	...
	$the_javascript  = "<script type='text/javascript' language='JavaScript'>\n";
	$the_javascript .= "function set_return(bug_id, bug_name) {\n";
	$the_javascript .= "	window.opener.document.".$_REQUEST['form'].".parent_name.value = bug_name;\n";
	$the_javascript .= "	window.opener.document.".$_REQUEST['form'].".parent_id.value = bug_id;\n";
	$the_javascript .= "}\n";
	$the_javascript .= "</script>\n";
	$button  = "<form border='0' action='index.php' method='post' name='form' id='form'>\n";
...
$ListView->setQuery($where, "", "name", "BUG");
$ListView->setModStrings($mod_strings);
$ListView->processListView($seed_object, "main", "BUG");
