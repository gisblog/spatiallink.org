 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ListView.php,v 1.3 2005/04/15 09:46:46 joey Exp $
...
$log = LoggerManager::getLogger('prospect_list');
...
if(isset($_REQUEST['query']))
{
	...
	if(isset($last_name) && $last_name != "") array_push($where_clauses, "prospects.last_name like '".PearDatabase::quote($last_name)."%'");
	if(isset($first_name) && $first_name != "")	array_push($where_clauses, "prospects.first_name like '".PearDatabase::quote($first_name)."%'");
	if(isset($do_not_call) && $do_not_call != "") array_push($where_clauses, "prospects.do_not_call = '".PearDatabase::quote($do_not_call)."'");
	if(isset($phone) && $phone != "") array_push($where_clauses, "(prospects.phone_home like '%".PearDatabase::quote($phone)."%' OR prospects.phone_mobile like '%".PearDatabase::quote($phone)."%' OR prospects.phone_work like '%".PearDatabase::quote($phone)."%' OR prospects.phone_other like '%".PearDatabase::quote($phone)."%' OR prospects.phone_fax like '%".PearDatabase::quote($phone)."%' OR prospects.assistant_phone like '%".PearDatabase::quote($phone)."%')");
	if(isset($email) && $email != "") array_push($where_clauses, "(prospects.email1 like '".PearDatabase::quote($email)."%' OR prospects.email2 like '".PearDatabase::quote($email)."%')");
	if(isset($assistant) && $assistant != "") array_push($where_clauses, "prospects.assistant like '".PearDatabase::quote($assistant)."%'");
	if(isset($email_opt_out) && $email_opt_out != "") array_push($where_clauses, "prospects.email_opt_out = '".PearDatabase::quote($email_opt_out)."'");
	if(isset($address_street) && $address_street != "") array_push($where_clauses, "(prospects.primary_address_street like '".PearDatabase::quote($address_street)."%' OR prospects.alt_address_street like '".PearDatabase::quote($address_street)."%')");
	if(isset($address_city) && $address_city != "") array_push($where_clauses, "(prospects.primary_address_city like '".PearDatabase::quote($address_city)."%' OR prospects.alt_address_city like '".PearDatabase::quote($address_city)."%')");
	if(isset($address_state) && $address_state != "") array_push($where_clauses, "(prospects.primary_address_state like '".PearDatabase::quote($address_state)."%' OR prospects.alt_address_state like '".PearDatabase::quote($address_state)."%')");
	if(isset($address_postalcode) && $address_postalcode != "") array_push($where_clauses, "(prospects.primary_address_postalcode like '".PearDatabase::quote($address_postalcode)."%' OR prospects.alt_address_postalcode like '".PearDatabase::quote($address_postalcode)."%')");
	if(isset($address_country) && $address_country != "") array_push($where_clauses, "(prospects.primary_address_country like '".PearDatabase::quote($address_country)."%' OR prospects.alt_address_country like '".PearDatabase::quote($address_country)."%')");
	if(isset($current_user_only) && $current_user_only != "") array_push($where_clauses, "prospects.assigned_user_id='$current_user->id'");

$seedProspect->custom_fields->setWhereClauses($where_clauses);
	...
	if (isset($assigned_user_id) && is_array($assigned_user_id))
	{
		...
		if ($count > 0 ) {
			...
			$where .= "prospects.assigned_user_id IN(";
...
if (!isset($_REQUEST['search_form']) || $_REQUEST['search_form'] != 'false') {
	...
	$search_form=new XTemplate ('modules/Prospects/SearchForm.html');
...
if(is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])){
	...
	if (isset($_REQUEST['advanced']) && $_REQUEST['advanced'] == 'true') {
		...
		$seedProspect->custom_fields->populateXTPL($search_form, 'search' );
...
$ListView->initNewXTemplate( 'modules/Prospects/ListView.html',$current_module_strings);
...
$ListView->setQuery($where, "", "last_name, first_name", "PROSPECT");
$ListView->processListView($seedProspect, "main", "PROSPECT");
?>
