 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: header.php,v 1.63 2005/04/29 22:16:31 joey Exp $
...
if (is_admin($current_user)) $xtpl->assign("ADMIN_LINK", "<a href='index.php?module=Administration&action=index' class='myArea'>".$app_strings['LBL_ADMIN']."</a>&nbsp;|&nbsp;");
...
foreach($preListHeader as $module_name)
{
		...
		if ($i==$max_tabs-1 and $numb_tabs>$max_tabs) {
			$xtpl->assign("MORE", "<td class=\"emptyTabSpace\"><img src='".$image_path."more.gif' alt='' align='absmiddle' id='MoreHandle' style='margin-bottom: 0px; margin-left:2px; cursor: hand;' align='absmiddle' onmouseover='tbButtonMouseOver(this.id,70,\"\",\"0\");'></td>");
		} 
		$xtpl->parse("main.tab");
	$i++;
}
...
if (count($history) > 0) {
	...
	$chopOff = substr($row['item_summary'], 0, 	25)."..";
