 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: header.php,v 1.28 2005/04/30 04:50:23 majed Exp $
	...
	foreach($pre_module_menu as $menu_item)
	{
		...
		if ($menu_item[1] != 'Deleted Items') {
			...
			if ($i==$max_sub_tabs-1 and $numb_tabs>$max_sub_tabs) {
				$xtpl->assign("SEPARATOR", "| <img src='".$image_path."moreSub.gif' width='10' height='18' border='0' align='absmiddle' style='cursor: hand;' onmouseover='tbButtonMouseOver(this.id,70,\"\",0);' id='SubMoreHandle'>");
			} else {
				if ($i!=$numb_tabs-1) {
...
$xtpl->parse("main.left_form.tab.sub_menu");
	...
	else 
	{
		...
		$xtpl->assign("MOUSEOVER", "onmouseover=\"this.style.background='#FFB5B5';this.style.cursor='hand';\" onmouseout=\"this.style.background='#ffffff';this.style.cursor='auto';\"");
