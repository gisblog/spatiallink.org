 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: SubPanelViewLeads.php,v 1.6 2005/04/20 07:53:00 joey Exp $
...
$button  = "<form action='index.php' method='post' name='LeadsForm' id='LeadsForm'>\n";
$button .= "<input type='hidden' name='module' value='Leads'>\n";
...
if ($currentModule == 'ProspectLists') 
	$button .= "<input title='".$app_strings['LBL_SELECT_BUTTON_TITLE']." ' accessKey='".$app_strings['LBL_SELECT_BUTTON_KEY']."' type='button' class='button' value='  ".$app_strings['LBL_SELECT_BUTTON_LABEL']."  ' name='button' LANGUAGE=javascript onclick='window.open(\"index.php?module=Prospects&action=PopupLeads&html=Popup_Leads_picker&form=LeadsForm&record=".$focus->id."&form_submit=true&query=true\",\"new\",\"width=600,height=400,resizable=1,scrollbars=1\");'>\n";
...
$ListView->initNewXTemplate('modules/Prospects/SubPanelViewLeads.html', $current_module_strings);

$ListView->xTemplateAssign("LEAD_LIST_RECORD", $focus->id);
...
$ListView->setQuery("", "", "last_name", "LEAD");
