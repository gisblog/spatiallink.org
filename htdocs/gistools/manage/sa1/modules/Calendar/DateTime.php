 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: DateTime.php,v 1.14 2005/03/23 19:07:21 robert Exp $
 ********************************************************************************/
...
class DateTime
{
	...
	function get_mysqli_date()
	{
		return $this->year."-".$this->zmonth."-".$this->zday;
	}
	function get_mysqli_time()
	{
  ...
  function parse_utc_date_time($str)
  {
    ...
    $date_arr = array(
      ...
      $date_time = & new DateTime($date_arr,true);
	...
	function getHashList($view, &$start_time,&$end_time)
	{
		...
		if ( $new_time->ts == $end_time->ts)
		{
		 ...
		 while( $new_time->ts < $end_time->ts)
		 {
		  ...
		  if ( $view == 'day')
		  {
		   $hash_list[] = $new_time->get_mysqli_date().":".$new_time->hour;
		   $arr['hour'] += 1;
		  }
		  else
		  {
		   $hash_list[] = $new_time->get_mysqli_date();
		   $arr['day'] += 1;
		  }
		  $new_time = & new DateTime($arr,true);
    }
		return $hash_list;
	}
