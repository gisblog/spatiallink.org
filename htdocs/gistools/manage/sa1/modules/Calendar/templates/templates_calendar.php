...
document.CallSave.date_start.value = "<?php echo $timedate->to_display_date($args['calendar']->date_time->get_mysqli_date(),false); ?>";
document.CallSave.time_start.value = "<?php echo $timedate->to_display_time($args['calendar']->date_time->get_mysqli_time(). ':00', false); ?>";
<?php
	}
}
?>
...
echo $callForm->getFormBody('', 'Calls','inlineCal'.template_echo_daily_view_24_hour($args).'CallSave',$timedate->to_display_date($args['calendar']->date_time->get_mysqli_date(), false),$timedate->to_display_time(template_echo_daily_view_24_hour ($args).':00:00',true, false)) ."<br>";
?></div>
...
$slice = $args['slice'];
if ( $slice->start_time->get_mysqli_date() == date('Y-m-d') )
{
return true;
}
}
...
function template_echo_slice_date(&$args)
{
	...
	$slice = $args['slice'];
		...
		if ( $slice->view != 'hour') {
			...
			echo $slice->start_time->get_mysqli_date();
			echo "','";
			echo $slice->start_time->get_mysqli_time();
			echo "'); return false;\">";*/
