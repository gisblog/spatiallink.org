 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: SubPanelViewOpportunity.php,v 1.25.6.2 2005/05/18 01:09:39 clint Exp $
...
$button .= "<td><input title='".$app_strings['LBL_SELECT_BUTTON_TITLE']."' accessKey='".$app_strings['LBL_SELECT_BUTTON_KEY']."' type='button' class='button' value='  ".$app_strings['LBL_SELECT_BUTTON_LABEL']."  ' name='button' LANGUAGE=javascript onclick='window.open(\"index.php?module=Contacts&action=Popup&html=Popup_picker&form=OpportunityDetailView&form_submit=true&query=true&account_id=$focus->account_id&account_name=".urlencode($focus->account_name)."\",\"new\",\"width=600,height=400,resizable=1,scrollbars=1\");'></td>\n";
$button .= "</tr></form></table>\n";

// Stick the form header out there.
echo get_form_header($current_module_strings['LBL_MODULE_NAME'], $button, false);

$xtpl=new XTemplate ('modules/Contacts/SubPanelViewOpportunity.html');
$xtpl->assign("MOD", $current_module_strings);
$xtpl->assign("APP", $app_strings);

$xtpl->assign("IMAGE_PATH", $image_path);
$xtpl->assign("RETURN_URL", "&return_module=Opportunities&return_action=DetailView&return_id=$focus->id");

$oddRow = true;
foreach($focus_list as $contact)
{
	if (isset($contact->opportunity_role)) 
	{
		$the_role = $app_list_strings['opportunity_relationship_type_dom'][$contact->opportunity_role];
	}
	else 
	{
		$the_role = '';
	...
	$contact_fields = array(
		...
		'OPPORTUNITY_ROLE' =>$the_role,
		'OPPORTUNITY_REL_ID' => $contact->opportunity_rel_id
	);
