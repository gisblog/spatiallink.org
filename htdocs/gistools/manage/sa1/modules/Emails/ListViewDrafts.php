 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ListViewDrafts.php,v 1.4 2005/04/14 18:03:43 lam Exp $
...
if (!isset($_REQUEST['search_form']) || $_REQUEST['search_form'] != 'false') {
	...
	$search_form->assign("SEARCH_ACTION", 'ListViewDrafts');
	...
	echo get_form_header($mod_strings['LBL_SEARCH_FORM_DRAFTS_TITLE']. $header_text, "", false);
...
if(isset($_REQUEST['query']))
{
	...
	$log->info("Here is the where clause for the list view: $where");
			...
			if($where != "")
			$where .= " and ";
			$where .= " emails.type='draft' OR ( emails.type='out' AND emails.status='send_error')";


global $email_title;
$display_title = $mod_strings['LBL_LIST_FORM_DRAFTS_TITLE'];
...
if(is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])){	
		$header_text = "&nbsp;<a href='index.php?action=index&module=DynamicLayout&from_action=ListViewDrafts&from_module=".$_REQUEST['module'] ."'>".get_image($image_path."EditLayout","border='0' alt='Edit Layout' align='bottom'")."</a>";
	}
$ListView->initNewXTemplate( 'modules/Emails/ListViewDrafts.html',$mod_strings);
$ListView->setHeaderTitle($display_title . $header_text );
$ListView->setQuery($where, "", "emails.date_entered", "DRAFTEMAIL");
$ListView->processListView($seedEmail, "main", "DRAFTEMAIL");
