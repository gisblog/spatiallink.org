...
$block2->sorting("organization_projects",$sortingUser->sor_organization_projects[0],"pro.name ASC",$sortingFields = array(0=>"pro.id",1=>"pro.name",2=>"pro.status",3=>"mem.login",4=>"pro.published"));
...
if ($comptListProjects != "0") {
	...
	$block2->labels($labels = array(0=>$strings["id"],1=>$strings["project"],2=>$strings["status"],3=>$strings["owner"],4=>$strings["project_site"]),"true");
...
//if mantis bug tracker enabled
if ($enableMantis == "true") {
	$block2->paletteScript(4,"bug",$pathMantis."login.php?url=http://{$HTTP_HOST}{$_SERVER['REQUEST_URI']}&f_username=$loginSession&f_password=$passwordSession","false,true,false",$strings["bug"]);
}
