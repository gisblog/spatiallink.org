...
$tmpquery = "WHERE tea.member = '$idSession' AND pro.status IN(0,2,3) AND pro.published = '0' ORDER BY pro.name";
...
echo "<tr class=\"$class\" onmouseover=\"this.style.backgroundColor='".$block1->highlightOn."'\" onmouseout=\"this.style.backgroundColor='".$highlightOff."'\"><td width=\"30%\"><a href=\"home.php?$transmitSid&updateProject=true&project=".$listProjects->tea_pro_id[$i]."\">".$listProjects->tea_pro_name[$i]."</a></td><td>$priority[$idPriority]</td><td>$status[$idStatus]</td></tr>";
}
echo "</table>
<hr />\n";
} else {
...
<tr><th nowrap class=\"FormLabel\">".$strings["created"]." :</th><td>&nbsp;".createDate($projectDetail->pro_created[0],$timezoneSession)."</td></tr>
<tr><th nowrap class=\"FormLabel\">".$strings["modified"]." :</th><td>&nbsp;".createDate($projectDetail->pro_modified[0],$timezoneSession)."</td></tr>
</table>";
