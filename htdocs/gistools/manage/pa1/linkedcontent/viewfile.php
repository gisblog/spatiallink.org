...
if ($allowPhp == "false") {
	...
	$n.rsion = $fileDetail->fil_vc_version[0] + $change_file_version;
	if ($copy == "true") {
		$name = $upload_name;
		$tmpquery = "UPDATE ".$tableCollab["files"]." SET date='$dateheure',size='$size',comments='$c',comments_approval='',approver='',date_approval='',status='$statusField',vc_version='$n.rsion' WHERE id = '$id'";
		connectSql("$tmpquery");
