...
#Status page: 0
	...
	function db_connect($p_hostname="", $p_username="",
		...
		$t_result = mysqli_connect(  $p_hostname.":".$p_port,
	...
	function db_pconnect($p_hostname="localhost", $p_username="root",
		...
		$t_result = mysqli_pconnect(  $p_hostname.":".$p_port,
									$p_username, $p_password );
	...
	function db_query( $p_query ) {
		...
		$t_result = mysqli_query($varconnect,  $p_query );
	...
	function db_select_db( $p_db_name ) {
		return mysqli_select_db( $p_db_name );
	}
	# --------------------
	...
	function db_result( $p_result, $p_index1=0, $p_index2=0 ) {
		if ( $p_result && ( db_num_rows( $p_result ) > 0 ) ) {
			return mysqli_result( $p_result, $p_index1, $p_index2 );
		} else {
			return false;
		}
	}
	...
	function db_error_msg() {
		return mysqli_error($varconnect);
	}
	# --------------------
