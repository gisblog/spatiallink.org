...
if (file_exists("modules/PhpCollab/pnversion.php")) {
	...
	if (mysqli_errno() != 0){ exit('<br><b>PANIC! <br> Error during connection on server MySQL.</b><br>'); }
	mysqli_select_db(MYDATABASE, $my);
	if (mysqli_errno() != 0){ exit('<br><b>PANIC! <br> Error during selection database.</b><br>'); }
	for($con = 0; $con < count($SQL); $con++){
	    mysqli_query($varconnect, $SQL[$con]);
	    //echo $SQL[$con] . '<br>';
	    if (mysqli_errno() != 0){ exit('<br><b>PANIC! <br> Error during the update of the database.</b><br> Error: '. mysqli_error($varconnect)); }
	}
	}
