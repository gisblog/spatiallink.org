...
#Status page: 0
/* $Id: defines.lib.php,v 1.17 2002/01/06 19:04:02 lem9 Exp $ */
...
if (!defined('PMA_MYSQL_INT_VERSION') && isset($userlink)) {
    if (!empty($server)) {
        $result = mysqli_query($varconnect, 'SELECT VERSION() AS version');
        if ($result != FALSE && @mysqli_num_rows($result) > 0) {
            $row   = mysqli_fetch_array($result);
            $match = explode('.', $row['version']);
        } else {
            $result = @mysqli_query($varconnect, 'SHOW VARIABLES LIKE \'version\'');
            if ($result != FALSE && @mysqli_num_rows($result) > 0){
