...
#Status page: 0
/* $Id: common.lib.php,v 1.47 2001/12/29 17:25:16 loic1 Exp $ */
...
if (!defined('PMA_COMMON_LIB_INCLUDED')){
    ...
    {
        if (!$error_message) {
            $error_message = mysqli_error($varconnect);
        }
     ...
     */
    $connect_func = ($cfgPersistentConnections) ? 'mysqli_pconnect' : 'mysqli_connect';
    $dblist       = array();
     ...
     */
        ...
        if ($cfgServer['stduser'] != '') {
            ...
            if ($dbh == FALSE) {
                if (mysqli_error($varconnect)) {
                    $conn_error = mysqli_error($varconnect);
        ...
        if ($dblist_cnt) {
            ...
            for ($i = 0; $i < $dblist_cnt; $i++) {
                if ($is_show_dbs && ereg('(^|[^\])(_|%)', $dblist[$i])) {
                    ...
                    $rs          = mysqli_query($varconnect, $local_query, $dbh);
                    // "SHOW DATABASES" statement is disabled
                    if ($i == 0
                        && (mysqli_error($varconnect) && mysqli_errno() == 1045)) {
                        $true_dblist[] = str_replace('\\_', '_', str_replace('\\%', '%', $dblist[$i]));
                        $is_show_dbs   = FALSE;
                    }
                    // Debug
                    // else if (mysqli_error($varconnect)) {
        ...
        // databases in the "mysql" db
            ...
            $rs         = mysqli_query($varconnect, $auth_query, $dbh); // Debug: or PMA_mysqlDie('', $auth_query, FALSE);
        } // end if

        // Access to "mysql" db allowed -> gets the usable db list
        if (!$dblist_cnt && @mysqli_numrows($rs)) {
            ...
            if ($row['Select_priv'] != 'Y') {
                ...
                $rs          = mysqli_query($varconnect, $local_query, $dbh); // Debug: or PMA_mysqlDie('', $local_query, FALSE);
                if (@mysqli_numrows($rs)) {
    ...
    function PMA_availableDatabases($error_url = '')
    {
        ...
        if ($num_dbs) {
            $true_dblist = array();
            for ($i = 0; $i < $num_dbs; $i++) {
                $dblink  = @mysqli_select_db($dblist[$i]);
                if ($dblink) {
        ...
        //    on the server
            ...
            $real_num_dbs = 0;
            for ($i = 0; $i < $num_dbs; $i++) {
                $db_name_tmp = mysqli_dbname($dbs, $i);
                $dblink      = @mysqli_select_db($db_name_tmp);
                if ($dblink) {
                    $dblist[] = $db_name_tmp;
                    $real_num_dbs++;
                }
    ...
    {
        $result = mysqli_query($varconnect, 'SELECT COUNT(*) AS num FROM ' . PMA_backquote($db) . '.' . PMA_backquote($table));
        $num    = mysqli_result($result, 0, 'num');
...
<div align="<?php echo $GLOBALS['cell_align_left']; ?>">
    ...
    function PMA_showDocuShort($link)
    {
        if (!empty($GLOBALS['cfgManualBaseShort'])) {
            return '[<a href="' . $GLOBALS['cfgManualBaseShort'] . '/' . $link .'" target="mysqli_doc">' . $GLOBALS['strDocu'] . '</a>]';
        }
