...
#Status page: 0
/* $Id: build_dump.lib.php,v 1.15 2002/01/03 14:38:04 loic1 Exp $ */
...
if (!defined('PMA_BUILD_DUMP_LIB_INCLUDED')){
    ...
    function PMA_getTableDef($db, $table, $crlf, $error_url)
    {
        ...
        if (PMA_MYSQL_INT_VERSION >= 32321) {
            ...
            if ($use_backquotes) {
                mysqli_query($varconnect, 'SET SQL_QUOTE_SHOW_CREATE = 1');
            } else {
                mysqli_query($varconnect, 'SET SQL_QUOTE_SHOW_CREATE = 0');
            }
            $result = mysqli_query($varconnect, 'SHOW CREATE TABLE ' . PMA_backquote($db) . '.' . PMA_backquote($table));
        ...
        $result        = mysqli_query($varconnect, $local_query) or PMA_mysqlDie('', $local_query, '', $error_url);
    ...
    function PMA_getTableContentFast($db, $table, $add_query = '', $handler, $error_url)
    {
        ...
        if ($result != FALSE) {
            ...
            for ($j = 0; $j < $fields_cnt; $j++) {
                $field_set[$j] = PMA_backquote(mysqli_field_name($result, $j), $use_backquotes);
                $type          = mysqli_field_type($result, $j);
    ...
    function PMA_getTableContentOld($db, $table, $add_query = '', $handler, $error_url)
    {
        ...
        while ($row = mysqli_fetch_row($result)) {
            ...
            $table_list     = '(';
            for ($j = 0; $j < $fields_cnt; $j++) {
                $table_list .= PMA_backquote(mysqli_field_name($result, $j), $use_backquotes) . ', ';
            }
