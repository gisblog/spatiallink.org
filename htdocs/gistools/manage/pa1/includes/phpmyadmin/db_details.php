...
if (PMA_MYSQL_INT_VERSION >= 32303) {
    ...
    if ($cfgSkipLockedTables == TRUE && PMA_MYSQL_INT_VERSION >= 32330) {
        ...
        $result       = mysqli_query($varconnect, $local_query) or PMA_mysqlDie('', $local_query, '', $err_url_0);
                ...
                if ($result != FALSE && mysqli_num_rows($result) > 0) {
                    while ($tmp = mysqli_fetch_row($result)) {
                        if (!isset($sot_cache[$tmp[0]])) {
                            ...
                            $sts_result  = mysqli_query($varconnect, $local_query) or PMA_mysqlDie('', $local_query, '', $err_url_0);
...
$result       = @mysqli_query($varconnect, 'USE mysql');
$is_superuser = (!mysqli_error($varconnect));
