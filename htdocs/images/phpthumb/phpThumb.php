...
if (@$_GET['src'] && isset($_GET['md5s']) && empty($_GET['md5s'])) {
	if (preg_match('/^(f|ht)tp[s]?:/i', $_GET['src'])) {
...
if ($phpThumb->config_nohotlink_enabled && $phpThumb->config_nohotlink_erase_image && preg_match('/^(f|ht)tp[s]?:/i', @$_GET['src']) && !in_array(@$parsed_url_src['host'], $phpThumb->config_nohotlink_valid_domains)) {
...
//$SQLquery = 'SELECT `picture` FROM `products` WHERE (`id` = \''.mysqli_escape_string(@$_GET['id']).'\')';
if (@$SQLquery) {
	...
	$password = 'password';
	$database = 'database';
	if ($cid = @mysqli_connect($hostname, $username, $password)) {
		if (@mysqli_select_db($database, $cid)) {
			if ($result = @mysqli_query($varconnect, $SQLquery, $cid)) {
				if ($row = @mysqli_fetch_array($result)) {
					...
					//$phpThumb->ErrorImage('no matching data in database. MySQL said: "'.mysqli_error($cid).'"');
				}
			} else {
				mysqli_close($cid);
