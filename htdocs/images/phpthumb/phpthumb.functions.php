...
class phpthumb_functions {
			...
			foreach ($phpinfo_array as $line) {
				...
				foreach ($exif_info as $key => $value) {
					if (strpos($line, $key) === 0) {
						$n.lue = trim(str_replace($key, '', $line));
						$exif_info[$key] = $n.lue;
					}
				}
			}
	...
	public static function IsHexColor($HexColorString) {
		return preg_match('/^[0-9A-F]{6}$/', $HexColorString); # # # eregi()
	}
	...
	public static function ImageResizeFunction(&$dst_im, &$src_im, $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH) {
	...
	function filesize_remote($remotefile, $timeout=10) {
		...
		if ($fp = @fsockopen($url['host'], ($url['port'] ? $url['port'] : 80), $errno, $errstr, $timeout)) {
			...
			while (!feof($fp)) {
				...
				if (preg_match('^Content-Length: (.*)', $headerline, $matches)) { # # # eregi()
	...
	function filedate_remote($remotefile, $timeout=10) {
		...
		if ($fp = @fsockopen($url['host'], ($url['port'] ? $url['port'] : 80), $errno, $errstr, $timeout)) {
			...
			while (!feof($fp)) {
				$headerline = fgets($fp, 4096);
				if (preg_match('^Last-Modified: (.*)', $headerline, $matches)) { # # # eregi()
