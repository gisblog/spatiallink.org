...
$result_qsearch = mysqli_query($varconnect, $query_qsearch) or die("Operation Failure :" . mysqli_error($varconnect));
/*	done */
...
$num_rows = mysqli_num_rows($result_qsearch);
if ($num_rows == 0) {
	...
	<!--include cell table: layout. class needs to be defined for every cell table or else IE will NOT render correctly-->
		...
		$sn = 1;
		//
		while ($row_qsearch = mysqli_fetch_array($result_qsearch)) {
			//	fee
