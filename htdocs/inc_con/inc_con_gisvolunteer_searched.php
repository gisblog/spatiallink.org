...
if ($searchbuffer == "5") {
	...
	include '/opt/bitnami/apache2/htdocs/inc_con/inc_con_gisvolunteer_searched_co.php';
} else {
	...
	if ($cocode == "256") {
		...
		$result_country = mysqli_query($varconnect, $query_country) or die("Operation Failure :" . mysqli_error($varconnect));
		//	check for NULL: NA
	...
	$result_search = mysqli_query($varconnect, $query_search) or die("Operation Failure :" . mysqli_error($varconnect));
	/*	done */
	...
	if ($num_rows == 0) {
		...
		$filehandle1 = fopen("/opt/bitnami/apache2/htdocs/txt/volunteer/automail_".$_POST[email]."_".$varip.".txt", "w");
		...
		<!--include cell table: layout. class needs to be defined for every cell table or else IE will NOT render correctly-->
			...
			//
			//	gmaps
			$filehandle2a = fopen("/opt/bitnami/apache2/htdocs/txt/volunteer/gmaps_".$filename_distinct.".xml", "a");
			$filecontent2a = "<markers>\r\n";
			fwrite($filehandle2a, $filecontent2a);	
			//
			while ($row_search = mysqli_fetch_array($result_search)) {
				/*	mysqli_fetch_array() consists of many arrays, like $zipcode. within the while{}, you can extract some individual row values. outside the while{}, $zipcodepieces will return the last value assigned:
				...
				refer below and [http://us4.php.net/mysqli_fetch_array] */
				...
				//
				//	gmaps
				$filehandle2b = fopen("/opt/bitnami/apache2/htdocs/txt/volunteer/gmaps_".$filename_distinct.".xml", "a");
				...
				//
				//	tms
				$filehandle3 = fopen("/opt/bitnami/apache2/htdocs/txt/volunteer/tms_".$filename_distinct.".txt", "a");
			...
			//	gmaps
			$filehandle2c = fopen("/opt/bitnami/apache2/htdocs/txt/volunteer/gmaps_".$filename_distinct.".xml", "a");
