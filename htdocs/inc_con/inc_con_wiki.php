	...
	<td colspan="2" class="medium">
		...
		if ($filehandle = opendir(".".$data_dir)) {
			while (false !== ($filename = readdir($filehandle))) {
				if ($filename != "." && $filename != "..") {
					...
					if (file_exists("/opt/bitnami/apache2/htdocs/gistools/discuss/wiki".$history_dir.$filename)) {
		...
		<?php
		// include rss
		include '/opt/bitnami/apache2/htdocs/inc_con/rss/rss_wiki.spatial.php';
		?>
		<!--<br />-->
		<br />
		&#8226;&nbsp;Related::
