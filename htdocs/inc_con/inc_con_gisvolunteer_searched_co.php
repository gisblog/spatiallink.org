...
$result_search_country = mysqli_query($varconnect, $query_search_country) or die("Operation Failure :" . mysqli_error($varconnect));
/*	done */
...
$num_rows = mysqli_num_rows($result_search_country);
if ($num_rows == 0) {
	...
	<!--include cell table: layout. class needs to be defined for every cell table or else IE will NOT render correctly-->
		...
		//
		while ($row_search_country = mysqli_fetch_array($result_search_country)) {
			/*	mysqli_fetch_array(): refer to inc_con_gisvolunteer_searched_co.php	*/
			//	gmaps
